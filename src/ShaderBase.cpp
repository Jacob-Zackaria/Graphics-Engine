...
ShaderBase::ShaderBase(ID3D11Device* device, ID3D11DeviceContext* newContext, WCHAR* filename, LPCSTR vsModel, LPCSTR psModel)
	: mDevice(device),
	name(nullptr),
	pNext(nullptr),
	pPrev(nullptr)
...
const bool ShaderBase::CompareName(char* newName)
{
	char* i = this->name;
	char* j = newName;
	while (*i != '\0' && *j != '\0')
...
char* ShaderBase::getName() const
{
	return this->name;
}

void ShaderBase::setName(char* inName)
{
	this->name = inName;
}

//-----links----
void ShaderBase::setNext(ShaderBase* newNext)
{
	this->pNext = newNext;
}

void ShaderBase::setPrev(ShaderBase* newPrev)
{
	this->pPrev = newPrev;
}

ShaderBase* ShaderBase::getNext() const
{
	return (this->pNext);
}

ShaderBase* ShaderBase::getPrev() const
{
	return (this->pPrev);
}